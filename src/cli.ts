...
program
  ...
  .action(async (options) => {
    try {
      console.log(chalk.blue(`[json-server-plus v${packageJson.version}] 🚀 Iniciando servidor...`));
      await startServer(options.file, parseInt(options.port), options.host);
      console.log(chalk.green(`[json-server-plus v${packageJson.version}] ✅ Servidor rodando em http://${options.host}:${options.port}`));
    } catch (error) {
      console.error(chalk.red(`[json-server-plus v${packageJson.version}] ❌ Erro ao iniciar o servidor:`), error);
      process.exit(1);
    }
  });
